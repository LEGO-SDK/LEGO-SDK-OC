...
- (void)requestAsynchronize:(LGORequestableAsynchronizeBlock)callbackBlock{
    if ([self.request.opt isEqualToString:@"add"]){
        id observer = [[NSNotificationCenter defaultCenter] addObserverForName:self.request.name object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            [[NSNotificationCenter defaultCenter] postNotificationName:self.request.name object:self.request.aPostObject userInfo:self.request.aPostUserInfo];
        }];
    }
}
