...
- (void)notifyDataModelDidChanged:(NSString *)dataKey dataValue:(id)dataValue {
    ...
    if ([NSJSONSerialization isValidJSONObject:dataValue]) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
          [self
              evaluateJavaScript:[NSString stringWithFormat:@"if (typeof JSDataModel != 'undefined'){JSDataModel['%@'] "
                                                            @"= JSON.parse(decodeURIComponent('%@'))}",
                                                            dataKey, jsonString]
               completionHandler:^(id _Nullable _, NSError *_Nullable error) {
                 [self evaluateJavaScript:[NSString stringWithFormat:@"(typeof JSDataModelDidChanged != 'undefined') "
                                                                     @"&& JSDataModelDidChanged('%@') ",
                                                                     dataKey]
