    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        if (!((NSNumber *)value).boolValue) {
            ...
            [[UIApplication sharedApplication]
                setStatusBarHidden:self.lgo_statusBarHidden
...
- (UIStatusBarStyle)lgo_statusBarStyle {
    return objc_getAssociatedObject(self, &kStatusBarStyleIdentifierKey) == nil
               ? UIStatusBarStyleDefault
               : [objc_getAssociatedObject(self, &kStatusBarStyleIdentifierKey) integerValue];
}

- (void)setLgo_statusBarStyle:(UIStatusBarStyle)lgo_statusBarStyle {
    objc_setAssociatedObject(self, &kStatusBarStyleIdentifierKey, @(lgo_statusBarStyle),
...
- (BOOL)lgo_statusBarHidden {
    return objc_getAssociatedObject(self, &kStatusBarHiddenIdentifierKey) == nil
               ? NO
               : [objc_getAssociatedObject(self, &kStatusBarHiddenIdentifierKey) boolValue];
}

- (void)setLgo_statusBarHidden:(BOOL)lgo_statusBarHidden {
    objc_setAssociatedObject(self, &kStatusBarHiddenIdentifierKey, @(lgo_statusBarHidden),
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
