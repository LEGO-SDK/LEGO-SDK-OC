...
- (void)lgo_openWebViewWithRequest:(NSURLRequest *)request args:(NSDictionary *)args {
    if (self.lgo_webView == nil) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
            ...
            if ([[[(WKWebView *)self.lgo_webView configuration] userContentController]
                    isKindOfClass:[LGOJavaScriptUserContentController class]]) {
                [(LGOJavaScriptUserContentController *)[[(WKWebView *)self.lgo_webView configuration]
                    userContentController] addPrescripts];
            }
            [(WKWebView *)self.lgo_webView loadRequest:request];
        } else {
            self.lgo_webView = NSClassFromString(@"LGOWebView")
